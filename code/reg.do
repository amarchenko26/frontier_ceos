...
* keep resource endowment industries + hospitality 
	keep if foodtobacco == 1 | restaurantslodging == 1
	reg ceos_norm c.tfe##c.lnpop, robust cluster(fips)
	eststo i
restore 
...
estout a b c d e f g h i using "/Users/anyamarchenko/Documents/GitHub/frontier_ceos/output/tables/table1.tex", style(tex) replace label starlevels(* .1 ** .05 *** .01 ) cells("b(fmt(4)star)" "se(fmt(4)par)") ///
	...
	"\toprule" ///
	"& \multicolumn{2}{c}{|--------- \textbf{CEO counts} --------|}  & \multicolumn{7}{c}{---------------------------------------------------- \textbf{CEOs per 10,000 people} -------------------------------------------} \\ & All counties & All counties & All counties & All counties & All counties (State FE) & \multicolumn{1}{c}{\shortstack{Small \\ counties}} & \multicolumn{1}{c}{\shortstack{Core \\ states}} & \multicolumn{1}{c}{\shortstack{Resource \\ Industries}} &\multicolumn{1}{c}{\shortstack{Hospitality \\ Industries}}   \\")  ///
	posthead("\midrule") ///
